...
def bucket_prefix_match(bucket_check, bucket_map, object=""):
    log.debug(f"bucket_prefix_match(): checking if {bucket_check} matches {bucket_map} w/ optional obj '{object}'")
    if bucket_check == bucket_map.split('/')[0] and object.startswith("/".join(bucket_map.split('/')[1:])):
        log.debug(f"Prefixed Bucket Map matched: s3://{bucket_check}/{object} => {bucket_map}")
        return True
    return False

def check_private_bucket(bucket, b_map, object=""):
    ...
    if 'PRIVATE_BUCKETS' in b_map:
        for priv_bucket in b_map['PRIVATE_BUCKETS']:
            if bucket_prefix_match(bucket, prepend_bucketname(priv_bucket), object):
...
def check_public_bucket(bucket, b_map, object=""):
    ...
    if 'PUBLIC_BUCKETS' in b_map:
        ...
        for pub_bucket in b_map['PUBLIC_BUCKETS']:
            if bucket_prefix_match(bucket, prepend_bucketname(pub_bucket), object):
