...
def get_urs_url(ctxt: dict, to: Optional[str] = None) -> str:
    ...
    log.debug(
        "if no domain name set: {}.execute-api.{}.amazonaws.com/{}".format(
            ctxt["apiId"], os.getenv("AWS_DEFAULT_REGION", "<region>"), ctxt["stage"]
        )
    )
...
def get_profile(
    ...
    try:
        ...
        if (
            not temptoken
        ):  # This keeps get_new_token_and_profile() from calling this over and over
...
def user_in_group_list(private_groups: list, user_groups: list) -> bool:
    client_id = get_urs_creds()["UrsId"]
    log.info(
        "Searching for private groups {0} in {1}".format(private_groups, user_groups)
    )

    group_names = {
        group["name"] for group in user_groups if group["client_id"] == client_id
    }
...
def user_in_group_urs(
    ...
    if not refresh_first:
        ...
        log.debug(
            f"Could not validate user {user_id} belonging to groups {private_groups}, attempting profile refresh"
        )

        return user_in_group_urs(
            private_groups, user_id, {}, refresh_first=True, aux_headers=aux_headers
        )
    log.debug(
        "Even after profile refresh, user {0} does not belong to groups {1}".format(
            user_id, private_groups
        )
    )
...
def user_in_group(
    ...
    if not in_group and not refresh_first:
        ...
        user_profile = get_profile(
            user_profile.user_id, user_profile.token, aux_headers=aux_headers
        )
...
def get_urs_creds() -> dict:
    ...
    if not ("UrsId" in secret and "UrsAuth" in secret):
        log.error(
            'AWS secret {} does not contain required keys "UrsId" and "UrsAuth"'.format(
                secret_name
            )
        )
...
def do_login(
    args, context, jwt_manager: JwtManager, cookie_domain="", aux_headers=None
):
    aux_headers = aux_headers or {}  # A safer default
    ...
    if args.get("error", False):
        contentstring = (
            'An error occurred while trying to log into URS. URS says: "{}". '.format(
                args.get("error", "")
            )
        )
    ...
    if user_profile is not None:
        ...
        headers.update(
            jwt_manager.get_header_to_set_auth_cookie(user_profile, cookie_domain)
        )
        return 301, {}, headers
