...
class TemplateManager:
    ...
    def download_templates(self):
        ...
        try:
            ...
            log.warning(
                "Failed to download HTML templates from %s", s3_uri, exc_info=True
            )
        finally:
...
def make_jwt_payload(payload: dict, algo: str = JWT_ALGO) -> str:
    try:
        ...
        log.info(
            return_timing_object(
                service="jwt", endpoint="jwt.encode()", duration=duration(timer)
            )
        )
        return encoded
    except KeyError as e:
...
def decode_jwt_payload(jwt_payload: str, algo: str = JWT_ALGO) -> dict:
    try:
        ...
        log.info(
            return_timing_object(
                service="jwt", endpoint="jwt.decode()", duration=duration(timer)
            )
        )
...
def make_set_cookie_headers_jwt(
    payload: dict,
    expdate: str = "",
    cookie_domain: str = "",
) -> dict:
    ...
    headers = {
        "SET-COOKIE": f"{JWT_COOKIE_NAME}={jwt_payload}; Expires={expdate}; Path=/{cookie_domain_payloadpiece}"
    }
    return headers
...
def is_jwt_blacklisted(decoded_jwt: dict) -> bool:
    try:
        ...
        if user_blacklist_time is not None:
            ...
            log.debug(
                f"JWT was minted @:  {jwt_mint_time}, the Blacklist is for cookies BEFORE: {user_blacklist_time}"
            )

            if user_blacklist_time >= jwt_mint_time:
                log.info(
                    f"User {urs_user_id}'s JWT was minted before blacklist date and is INVALID"
                )
                return True
            else:
                log.info(
                    f"User {urs_user_id}s JWT was minted AFTER blacklist date and is still VALID"
                )
...
def set_jwt_blacklist() -> dict:
    ...
    if JWT_BLACKLIST and time() - JWT_BLACKLIST["timestamp"] <= (
        10 * 60
    ):  # If cached in the last 10 minutes
    ...
    log.info(
        return_timing_object(
            service="blacklist", endpoint=endpoint, duration=duration(timer)
        )
    )
